...
export default async function MissionPage({
  ...
  const missionNodes = getContainerNodes(
    mission,
    'mission',
    mission.key,
  );
  ...
  if (nodeKey) {
    ...
    if (
      !initialNode ||
      initialNode.container.type !== 'mission'
    ) {
      notFound();
    }
  } else {
        ...
        <div className="grid gap-12 xl:grid-cols-[minmax(0,1fr)_18rem]">
          ...
          <section className="min-w-0 max-w-4xl">
            <header className="mb-10">
              ...
              {mission.bigQuestion && (
                <blockquote className="mt-10 max-w-3xl border-l-2 border-primary pl-6 text-xl leading-8 italic sm:text-2xl sm:leading-9">
                  {mission.bigQuestion}
                </blockquote>
              )}
            </header>
